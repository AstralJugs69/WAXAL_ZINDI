...
def run_training(args, config, is_tpu=False, index=0):
    ...
    train_path = f"{outputs_dir}/temp_train_fold{args.fold}.csv"
    val_path = f"{outputs_dir}/temp_val_fold{args.fold}.csv"
...
def main():
    ...
    if args.tpu:
        ...
        import time
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
        done_flag_path = f"{get_outputs_dir()}/temp_done_fold{args.fold}.txt"
